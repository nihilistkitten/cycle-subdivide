...
const MAX_SPEED = 0.7; // Cross seven faces every 10 frames.
...
class Cycle {
  ...
  resetOn(surface, faceid = 0) {
    ...
    // reset colors
    for (let f of this.visited) {
      surface.paint(f, {
        r: 0.25 * Math.random() * 0.25,
        g: 0.25 + Math.random() * 0.25,
        b: 0.25 + Math.random() * 0.25,
      });
    }
    this.visited = [];
  }
  ...
  advanceEdgeFace() {
    ...
    //
    if (this.visited.includes(this.face)) {
      alert("You lost! Try not to touch your own trail!");
      console.log(this.surface);
      this.resetOn(this.surface);
      // gSurfaces.delete(this.surface);
      // gSurfaces.set(this.surface);
      // chooseSurface(this.surface);
      // refinement.glCompile();
      // refinement.glCompileMesh();
    }
    this.visited.push(this.face);
