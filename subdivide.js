...
EPSILON = 1.0e-8;
function BETA(k) {
  return 5 / 8 - (3 / 8 + (1 / 4) * Math.cos((2 * Math.PI) / k)) ** 2;
}
...
class Vertex {
  ...
  getNeighbors() {
    //
    // Get the neighbors of the vertex by walking around the adjacent faces.
    //
    let edge = this.edge;
    let out = [];
    do {
      out.push(edge.target);
      // walks the face, the switches to the adjacent face
      edge = edge.prev.twin;
  ...
  isValid() {
    return this.checkValid().every((b) => b);
  }
}
...
class Edge {
  ...
  checkValid() {
    return [
      ...
      this.twin.source === this.target,
      this.twin.target === this.source,
    ];
  }
...
class Face {
  ...
  getVertices() {
    //
    // Returns the ordered triple of vertices around the face as an array of
    // size 3.
    //

    const p0 = this.edge.prev.target;
    const p1 = this.edge.target;
    const p2 = this.edge.next.target;
    return [p0, p1, p2];
  }
}
...
class Surface {
  ...
  subdivide() {
    ...
    for (let vertex of S.allVertices()) {
      // make clone vertices
      const neighbors = vertex.getNeighbors().map((v) => v.position);
      const degree = neighbors.length;

      const scalars = Array(degree).fill(BETA(degree) / degree);
      const new_position = vertex.position.combos(scalars, neighbors);

      const clone = R.makeVertex(new_position);
      vertex.clone = clone;
    }

    for (let edge of S.allEdges()) {
      if (edge.twin.split) {
        edge.split = edge.twin.split;
      } else {
        // make split vertices
        const [p0, p1] = edge.getPoints();
        const q0 = edge.next.target.position;
        const q1 = edge.twin.next.target.position;

        const new_point = p0.combos([0.375, 0.125, 0.125], [p1, q0, q1]);
        const split = R.makeVertex(new_point);
        edge.split = split;
      }
    }

    for (let face of S.allFaces()) {
      const [p0, p1, p2] = face.getVertices().map((v) => v.clone);
      ...
      R.makeFace(p0.id, m0.id, m2.id);
      R.makeFace(p1.id, m1.id, m0.id);
      R.makeFace(p2.id, m2.id, m1.id);
      R.makeFace(m0.id, m1.id, m2.id);
    }

    // for (let edge of R.allEdges()) {
    //   if (!edge.isValid()) {
    //     console.log(edge, edge.checkValid());
    //   }
    // }
    // for (let vertex of R.allVertices()) {
    //   if (!vertex.isValid()) {
    //     console.log(vertex, vertex.checkValid());
    //   }
    // }
